...
class KNN(Model):
    ...
    def knn_pred(self, K=200):
        ...
        pd_train = pd.DataFrame(list(cur_train))[[AUTHOR, TITLE, LBL]]
        pd_valid = pd.DataFrame(list(cur_valid))[[AUTHOR, TITLE, LBL]]
        pd_test = pd.DataFrame(list(cur_test))[[AUTHOR, TITLE, LBL]]
...
if __name__ == "__main__":
    ...
    knn.clean()
    dup_ratio = 0
    knn.sampling(dup_ratio=dup_ratio)
    knn.shuffle_label()
    rt = 5
    knn.split(train_count=int(35000 // 5 * rt * (1 + dup_ratio)), valid_count=int(10000 // 5 * rt * (1 + dup_ratio)),
              test_count=int(5000 // 5 * rt * (1 + dup_ratio)))
