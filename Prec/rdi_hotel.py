...
class Collective_opt(Model):
    ...
    def featuring(self, texts_train, texts_valid, texts_test):
        ...
        vocab_valid_test = set(vocab_test.keys()).union(vocab_test.keys())
    ...
    def _optimize(self, features, labels):
        ...
        def _opt(tau):
            ...
            for i in range(len(Z_idx)):
                ...
                if (N_tp / N_h < self.prec and N_h >= 200):
                    N_h = N_h - 1
                    break
                else:
                    theta_tmp = Z[Z_idx[i]]
...
if __name__ == "__main__":
    ...
    try:
        for (prec, q, tau) in itertools.product(prec_range, q_range, tau_range):
            print(prec, q, tau)
            res, res2 = hotel(cv12, tau=tau, prec=prec, q=q)
