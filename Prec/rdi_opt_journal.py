    ...
    def _optimize(self, features, labels):
        ...
        def _opt(tau):
            ...
            for i in range(len(Z_idx)):
                ...
                if (N_tp / N_h < self.prec and N_h > 400):
                    N_h = N_h - 1
                    break
                else:
                    theta_tmp = Z[Z_idx[i]]
...
def citeseer(cv, tau, prec, q=1.0):
    ...
    df_train = pd.DataFrame(list(train.find()))[[AUTHOR, TITLE, LBL]]
    df_valid = pd.DataFrame(list(valid.find()))[[AUTHOR, TITLE, LBL]]
    df_test = pd.DataFrame(list(test.find()))[[AUTHOR, TITLE, LBL]]

    y_train = df_train.pop(LBL)
    y_valid = df_valid.pop(LBL)
    y_test = df_test.pop(LBL)

    at_train = [' '.join(data) for data in df_train.values]
    at_valid = [' '.join(data) for data in df_valid.values]
    at_test = [' '.join(data) for data in df_test.values]
    ...
    prec_range = np.arange(.8, 0.81, 0.05) + model.delta(N_valid, 1, 0.1)
